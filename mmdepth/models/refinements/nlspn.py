...
class NLSPN(BaseModule):
    def __init__(self, 
                 ...
                 temperature=1,
                 init_cfg=None):
        super(NLSPN, self).__init__(init_cfg)
    ...
    def _get_offset_affinity(self, guidance, confidence=None, rgb=None):
        ...
        if self.affinity in ['AS', 'ASS', 'TC', 'TGASS']:
            ...
            if self.affinity in ['AS', 'ASS']:
                ...
                aff = torch.tanh(aff/self.temperature) / self.aff_scale_const
            elif self.affinity == 'TGASS':
                aff = torch.tanh(aff/self.temperature) / (self.aff_scale_const + 1e-8)
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        if self.conf_prop:
            local_offset = torch.zeros_like(offset)
            if self.legacy:
                for idx_off in range(0, self.num + 1):
                    ...
                    if ww == (self.k_f - 1) / 2 and hh == (self.k_f - 1) / 2:
                        continue
                    local_offset[:, 2*idx_off, :, :] = hh - (self.k_f - 1) / 2
                    local_offset[:, 2*idx_off+1, :, :] = ww - (self.k_f - 1) / 2
            ...
            for idx_off in range(0, self.num + 1):
                ...
                conf_tmp = deform_conv2d(confidence, offset_tmp, self.w_conf, self.b, self.stride, 0, self.dilation, mask=modulation_dummy)
    ...
    def _propagate_once(self, feat, offset, aff):
        feat = deform_conv2d(feat, offset, self.w, self.b, self.stride, self.padding, self.dilation, mask=aff)
    ...
    def forward(self, inputs:dict):
        feat_init = inputs['feat_init']
        guidance = inputs['affinity']
        confidence = inputs.get('confidence', None)
        feat_fix = inputs.get('feat_fix', None)
        rgb = inputs.get('img', None)
